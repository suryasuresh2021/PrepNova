          ...
          <div className="mt-8 space-y-10">
            {categories.map((cat) => {
              ...
              const categoryTests = (tests || []).filter(
                (t) =>
                  (t.topic_id && categoryTopicIds.has(t.topic_id)) ||
                  (!t.topic_id && categoryTopicNames.has(norm(t.topic)))
              );
                ...
                <div key={cat.id}>
                  <div className="flex flex-wrap items-center justify-between gap-2">
                    ...
                    <span className="font-body text-xs font-medium text-slate-500">
                      {categoryMaterials.length} material{categoryMaterials.length === 1 ? "" : "s"} ·{" "}
                  ...
                  <div className="mt-4 space-y-4">
                    ...
                    {(cat.topics || []).length === 0
                      ? generalMaterials.length === 0 && (
                          <p className="font-body text-sm text-slate-400">No topics added yet.</p>
                        )
